...
if [ "$confirmation" != "y" ]; then
    echo -e "${RED}Install dibatalkan. Tidak ada perubahan dalam ubuntu server anda.${NC}"
    /tmp/install.sh
    exit 1
fi
for ((i = 5; i >= 1; i--)); do
...
if ! check_node_version; then
    echo -e "${RED}================== Menginstall NodeJS ==================${NC}"
    curl -fsSL https://deb.nodesource.com/setup_20.x | sudo bash -
    sudo apt-get install -y nodejs
    echo -e "${RED}================== Sukses NodeJS ==================${NC}"
else
    NODE_VERSION=$(node -v | cut -d 'v' -f 2)
    echo -e "${RED}============================================================================${NC}"
    echo -e "${RED}============== NodeJS wis install ${NODE_VERSION}. ==============${NC}"
    echo -e "${RED}========================= Lanjut install GenieACS ==========================${NC}"
fi

#MongoDB
if !  systemctl is-active --quiet mongod; then
    echo -e "${RED}================== Menginstall MongoDB ==================${NC}"
...
if !  systemctl is-active --quiet genieacs-{cwmp,fs,ui,nbi}; then
    echo -e "${RED}================== Installing genieACS CWMP, FS, NBI, UI ==================${NC}"
...
echo -e "${RED}============================================================================${NC}"
echo -e "${RED}=================== VIRTUAL PARAMETER BERHASIL DI INSTALL. =================${NC}"
echo -e "${RED}===Jika ACS URL berbeda, silahkan edit di Admin >> Provosions >> inform ====${NC}"
echo -e "${RED}========== GenieACS UI akses port 3000. : http://$local_ip:3000 ============${NC}"
echo -e "${RED}=================== Informasi: Whatsapp 081947215703 =======================${NC}"
echo -e "${RED}============================================================================${NC}"
